...
//                  Modified from Kishan Bagaria's One Click Llama Button 💚
...
(function () {
    ...
    /* ==========================================================================
       ...
       Functions to retrieve CSRF token needed for giving Cake badges,
       including fallback method fetching from user page if token not found in DOM
       ========================================================================== */
    function getToken(doc) {
        // Check scripts for token variable
        let scripts = doc.scripts;
        for (let i = 0; i < scripts.length; i++) {
            if (scripts[i].innerHTML.includes('window.__CSRF_TOKEN__ ')) {
                let parts = scripts[i].innerHTML.split('window.__CSRF_TOKEN__ ');
                return parts[1].split(/'/)[1];
            }
        }
        // Fallback: look for hidden input in logout form
        const input = doc.querySelector("#logout-form input[type='hidden']");
        return input ? input.value : null;
    }

    // Asynchronously get cached or fresh CSRF token, with fallback fetch
    async function getCsrfToken() {
        let cached = cakeStorage('get', 'cached_csrf');
        if (cached) return cached;
        const csrfInput = document.querySelector("#logout-form input[type='hidden']");
        if (csrfInput) {
            cakeStorage('set', 'cached_csrf', csrfInput.value);
            return csrfInput.value;
        }
        // Fetch token from a known page as fallback
        const altUrl = 'https://www.deviantart.com/liamb135';
        try {
            const res = await fetch(altUrl);
            const htmlText = await res.text();
            const div = document.createElement('div');
            div.innerHTML = htmlText;
            const altInput = div.querySelector("#logout-form input[type='hidden']");
            if (altInput) {
                cakeStorage('set', 'cached_csrf', altInput.value);
                return altInput.value;
            }
        } catch { }
        return null; // Could not get token
    }
    ...
    function saveLastState(devName, className, title) {
        if (className !== 'unknown') cakeLastStates[devName] = { className, title };
    }
    ...
    async function cakeButtonClicked(event) {
        ...
        // Attempt to get CSRF token for API call
        ...
        if (repeatGive) {
            ...
            while (true) {
                ...
                await delay(2500); // Wait 5 seconds between gives
    ...
    function getGiveMenu(devName, callback) {
        getCsrfToken()
            .then(csrfToken => {
                if (!csrfToken) {
                    setButtonsState(devName, 'token_miss', TITLES.token_miss);
                    return;
                }
            ...
            .catch(() => setButtonsState(devName, 'token_miss', TITLES.token_miss));
    }
    ...
    /* ==========================================================================
       Section 10: DOM Scanning & Mutation Observer
    ...
    /* ==========================================================================
       Section 12: Main Entry Point & Error Handling
